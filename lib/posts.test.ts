...
describe('Blog logic', () => {
  describe('createSlug', () => {
    it('correctly converts the title to a slug', () => {
      expect(createSlug('Aby życie było ciut prostsze')).toBe(
        'aby-zycie-bylo-ciut-prostsze'
      )
    })
    ...
    it('removes special characters and extra spaces', () => {
      expect(createSlug('Prawda, czy nie prawda? - oto jest pytanie!')).toBe(
        'prawda-czy-nie-prawda-oto-jest-pytanie'
      )
    })
